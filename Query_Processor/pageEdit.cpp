...
namespace pageEdit {
    int getAndUpdateFreePage(std::string &page, int recordSize, bool isVar) {
        //nroDePares(3) header(3) freeSpace(3) id size id size id size id &
    ...
    std::string setNewSlotted(size_t blockSize) {
        ...
        outStream << std::string(blockSize - (NUM_RECORDS_SIZE+VAR_OFFSET_SIZE), '-');  // Restamos 6 para considerar los 6 bytes de la cabecera
    ...
    Cabecera leerCabecera(const std::string& data) {
        ...
        if (!data.empty()) {
            ...
            if (cabecera.numRegistros > 0) {
                size_t offsetStart = NUM_RECORDS_SIZE+VAR_OFFSET_SIZE; // Posición donde comienzan los offsets en la cadena
                size_t recordInfoLength = VAR_OFFSET_SIZE+VAR_LENGTH_SIZE; // Tamaño en caracteres para cada conjunto de offset y longitud

                for (size_t i = 0; i < cabecera.numRegistros; ++i) {
                    std::string offsetStr = data.substr(offsetStart + i * recordInfoLength, VAR_OFFSET_SIZE);
                    std::string lengthStr = data.substr(offsetStart + i * recordInfoLength + VAR_OFFSET_SIZE, VAR_LENGTH_SIZE);
    ...
    std::string insertSlotted(const std::string& data, const std::string& registro) {
        ...
        if ((cabecera.finFreeSpace - (cabecera.numRegistros * (VAR_OFFSET_SIZE+VAR_LENGTH_SIZE) + (NUM_RECORDS_SIZE+VAR_OFFSET_SIZE))) < registro.size()) {
