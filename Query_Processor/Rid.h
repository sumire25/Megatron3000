...
class RID {
 public:
  ...
  explicit RID(int64_t rid) : page_id_(static_cast<int>(rid >> 32)), slot_num_(static_cast<int>(rid)) {}
  ...
  inline auto GetSlotNum() const -> int { return slot_num_; }

  inline void Set(int page_id, int slot_num) {
    page_id_ = page_id;
    slot_num_ = slot_num;
  }
